...
fname = '/Users/uqdlevyb/Desktop/02_Data/02_Data_COSMOS/Behav_ALL/BHV_V9.xlsx';

sheet_name = 'ALL';
BHV_V9 = xls2struct(fname);
fields = fieldnames(BHV_V9);
for field = 1:length(fields)
    BHV_V9.(fields{field}) = cell2mat(BHV_V9.(fields{field}));
end;
%% PLAYING WITH THE DATA
figure
id_sbj = unique(BHV_V9.subjectID);
n_sbj = length(id_sbj);
n_tasks = length(unique(BHV_V9.task));
n_cue = length(unique(BHV_V9.cue));
K = nan(n_cue,n_sbj,n_tasks); % gaussian (von mises) distribution capturing the variability of the responses
pT = nan(n_cue,n_sbj,n_tasks); % precision target parameter (defined as pT is the probability of responding with the target value)
pN = nan(n_cue,n_sbj,n_tasks); % precision non target parameter (swap errors?) (defined as the probability of responding with a non-target value)
pU = nan( n_cue,n_sbj,n_tasks); % random (guesses) (defined as the probability of responding "randomly")
idx = 0;
for task = 1:n_tasks
     for cno = 1:n_cue
        for sno = 1:n_sbj
            idx_sbj = BHV_V9.subjectID == id_sbj(sno); % selecting the participant
            idx_task = BHV_V9.task == task; % selecting the task we want to fit
            idx_good_trl = BHV_V9.fixated == 1; % selecting only good trials
            idx_cue = BHV_V9.cue == cno; % selecting the correct cue (1/2)
            idx_tot = idx_good_trl & idx_task & idx_sbj & idx_cue;
            T = BHV_V9.target(idx_tot);
            T_rad = wrapToPi(T);
            X = BHV_V9.response(idx_tot);
            X_rad = wrapToPi(X);
            ...
            thickness= k2sd (K);% from JV10, transforms kappa to SD.
            ...
            BHV_V9.errMagn(idx_tot) = errMagn;
            BHV_V9.errMagn = BHV_V9.errMagn'; % Here, the values are 1x22448 and we want one column with several raw (22448 x 1) this line transposes them
...
for task = 1:n_tasks
    for cno = 1:n_cue
        for sno = 1:n_sbj
            ...
            errMagn_sub_cond_good = BHV_V9.errMagn(idx_tot); % contains the data for the correct subj no, the correct cond, the good trials
...
for task = 1:n_tasks
    for cno = 1:n_cue
        for sno = 1:n_sbj
            ...
            switch task
                case {1 2}
                    P(:,1)=BHV_V9.ori_c_A(idx_tot);
                    P(:,2)=BHV_V9.ori_c_B(idx_tot);
                    P(:,3)=BHV_V9.ori_c_C(idx_tot);
                    P(:,4)=BHV_V9.ori_u_A(idx_tot);
                    P(:,5)=BHV_V9.ori_u_B(idx_tot);
                    P(:,6)=BHV_V9.ori_u_C(idx_tot);
                case 3
                    P(:,1)=BHV_V9.loc_c_A(idx_tot);
                    P(:,2)=BHV_V9.loc_c_B(idx_tot);
                    P(:,3)=BHV_V9.loc_c_C(idx_tot);
                    P(:,4)=BHV_V9.loc_u_A(idx_tot);
                    P(:,5)=BHV_V9.loc_u_B(idx_tot);
                    P(:,6)=BHV_V9.loc_u_C(idx_tot);
            end
            C = zeros(sum(idx_tot),1);
            C = BHV_V9.response(idx_tot);
